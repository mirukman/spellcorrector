...
public class HangulCombinator {
    ...
    private String decompose(char hangul) {
        ...
        if(hangul < '가' || hangul > '힣') return String.valueOf(hangul);
        ...
        final int jungsungIndex = (diff - ((JONGSUNG_MAP.size() * JUNGSUNG_MAP.size()) * chosungIndex)) / JONGSUNG_MAP.size();
        jasoBuilder.append(JUNGSUNG_LIST[jungsungIndex]);

        final int jongsungIndex = (diff - ((JONGSUNG_MAP.size() * JUNGSUNG_MAP.size()) * chosungIndex) - (JONGSUNG_MAP.size() * jungsungIndex));
        if (jongsungIndex > 0) {
    ...
    public String compose(String str) {
        ...
        while(start < str.length()) {
            ...
            if(CHOSUNG_MAP.containsKey(str.charAt(start))) {
                ...
                if(start < str.length() && JUNGSUNG_MAP.containsKey(str.charAt(start))) {
                    ...
                    if(start < str.length() && JONGSUNG_MAP.containsKey(str.charAt(start)) &&
                            !(start + 1 < str.length() && JUNGSUNG_MAP.containsKey(str.charAt(start + 1)))) {
                        ...
                        if(str.charAt(start) != ' ') {
                            ...
                            if(start < str.length() &&
                                    ...
                                    !(start + 1 < str.length() && JUNGSUNG_MAP.containsKey(str.charAt(start + 1)))) {
                                String dualJongSung = String.valueOf(str.charAt(start - 1)) + str.charAt(start);
                                if(DUAL_JONGSUNG_MAP.containsKey(dualJongSung)) {
                                    result_char -= jongsungIdx;
                                    jongsungIdx = JONGSUNG_MAP.get(DUAL_JONGSUNG_MAP.get(dualJongSung).charAt(0));
                                    result_char += jongsungIdx;
                                    start++;
                                }
                            }
                    ...
                    //중성이 이중모음인지 체크
                        ...
                        String dualJungSung = String.valueOf(str.charAt(start - 1)) + str.charAt(start);
                        if(DUAL_JUNGSUNG_MAP.containsKey(dualJungSung)) {
                            result_char -= JONGSUNG_MAP.size() * jungsungIdx;
                            jungsungIdx = JUNGSUNG_MAP.get(DUAL_JUNGSUNG_MAP.get(dualJungSung).charAt(0));
                            result_char += JONGSUNG_MAP.size() * jungsungIdx;
                            start++;
            ...
            } else if(JUNGSUNG_MAP.containsKey(str.charAt(start)) &&
                    (start + 1 < str.length() && JUNGSUNG_MAP.containsKey(str.charAt(start + 1)))) {
